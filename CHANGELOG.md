...
- `debug-logging` GSettings key (default `false`) that gates all diagnostic
  output to the systemd journal. Toggle live with:
  ```sh
  gsettings set org.gnome.shell.extensions.github-notifications-redux \
      debug-logging true
  ```

### Changed

- All `console.error` call sites consolidated through a single
  `_logError()` helper, so the journal stays quiet for normal users while
  full diagnostics remain a one-line opt-in away.
- `disable()` now explicitly destroys `_label` and `_notifSection` and
  disconnects the menu `open-state-changed` and notification `activated`
  signals it tracks during `enable()` (matches the
  EGO-L-002 / EGO-L-003 review checks).

### Removed

- `install.sh` script — removed from the shipped package per the
  EGO-P-006 review check (use `nix run .#install` or copy the directory to
  `~/.local/share/gnome-shell/extensions/` manually instead).
- `schemas/gschemas.compiled` removed from the repository and added to
  `.gitignore`; GNOME compiles schemas at install time.

### Fixed

- Addresses all four warnings from the e.g.o. shexli review of v1:
  GO-A-004 (excessive ungated logging), EGO-P-006 (shipped build/compiled
  artifacts), EGO-L-002 (objects not destroyed), EGO-L-003 (signals not
  disconnected).
...
[1.2.0]: https://github.com/NelsonJeppesen/gnome-github-notifications-redux/compare/v1.1.0...v1.2.0
