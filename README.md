...
Copy the source tree to GNOME's local extensions directory and compile the
schema:

```sh
DEST=~/.local/share/gnome-shell/extensions/github-notifications-redux@jeppesen.io
mkdir -p "$DEST"
cp -r extension.js prefs.js metadata.json stylesheet.css \
      github-symbolic.svg schemas "$DEST/"
...
The extension stays silent in the journal by default. To capture diagnostic
output (HTTP errors, fetch failures, token problems, etc.), enable the
hidden `debug-logging` setting:
...
# Watch the log
journalctl -f -o cat /usr/bin/gnome-shell | grep "GitHub Notifications"
...
The change takes effect immediately — no need to reload the extension.
