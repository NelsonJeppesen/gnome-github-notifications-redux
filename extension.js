...
export default class GitHubNotificationsExtension extends Extension {
    ...
    disable() {
        ...
        if (this._notificationSignalIds) {
            for (const {notification, id} of this._notificationSignalIds) {
                try {
     ...
     *
     * Writes to the systemd journal only when the `debug-logging` GSettings
     * key is enabled.  This keeps the journal quiet for normal users while
     * still allowing detailed troubleshooting via:
     *
    ...
    _logError(msg) {
        if (this._debugLogging)
            console.error(`[GitHub Notifications] ${msg}`);
    }
    ...
    _initIndicator() {
        ...
        this._indicator.menu.addAction('Preferences', () => {
            this.openPreferences().catch(e =>
                this._logError(`Prefs error: ${e.message}`));
        });
    ...
    async _fetchNotifications() {
        ...
        try {
            ...
            if (status === Soup.Status.UNAUTHORIZED) {
                this._logError('401 Unauthorized – check token');
            ...
            this._logError(`HTTP ${status}`);
            this._scheduleFetch(this._getEffectiveInterval(), true);
        } catch (e) {
            this._logError(`Fetch error: ${e.message}`);
            this._scheduleFetch(this._getEffectiveInterval(), true);
        }
    }
    ...
    async _markAllRead() {
        ...
        try {
            ...
            if (status === Soup.Status.RESET_CONTENT ||
                ...
                this._logError(`Mark-all-read failed: HTTP ${status}`);
            }
        } catch (e) {
            this._logError(`Mark-all-read error: ${e.message}`);
        }
    }
    ...
    _sendDesktopNotification(count) {
        try {
            ...
            this._notificationSignalIds.push({notification, id: activatedId});

            source.addNotification(notification);
        } catch (e) {
            this._logError(`Desktop notification error: ${e.message}`);
        }
    }
    ...
    _openNotifications() {
        try {
            ...
            this._logError(`Cannot open URI: ${e.message}`);
        }
    }
    ...
    async _fetchReleaseTagUrl(apiUrl, repoFullName) {
        ...
        try {
            ...
            this._logError(`Failed to resolve release URL: ${e.message}`);
        }
    ...
    async _markThreadRead(notif) {
        ...
        try {
            ...
            if (status === Soup.Status.RESET_CONTENT ||
                ...
                this._logError(`Mark-thread-read failed: HTTP ${status}`);
            }
        } catch (e) {
            this._logError(`Mark-thread-read error: ${e.message}`);
        }
    }
}
